	...
	<view class="rtc" :style="background">
		...
		<view class="ctrl">
			<view v-for="(item, index) in ctrlList" :key="index" :class="['ctrl-box', margin(index)]">
				<view v-if="item.type === 'speaker'" :class="['ctrl-item', speaker ? 'active' : '']" @tap="tapCtrl(item.func, item.mode)">
				...
				<view v-if="item.type.includes('switch') || item.extend === 'switch-voice'" :class="['ctrl-item', item.type, item.extend ? 'switch-voice-box' : '', item.extend]" hover-class="hover_switch" hover-start-time="100" hover-stay-time="150">
				...
				<view v-if="!['voice', 'video', 'hang'].includes(item.type)" :class="['ctrl-item', isActive(item.mode)]" @tap="tapCtrl(item.func, item.mode)">
					<view :class="['iconfont', `icon-${item.type}`, 'uIcon']"></view>
				</view>
				<u-button v-else="" :type="item.type === 'hang' ? 'error' : 'primary'" :custom-style="ctrlBtn">
					<view :class="['iconfont', `icon-${item.type}`, item.type]"></view>
				</u-button>
				<view class="ctrl-text">
					{{ item.text }}
				</view>
				<!-- <view v-if="showSwitchVoice(index)" class="switch-voice-box ctrl-item switch-voice" hover-class="hover_switch" hover-start-time="100" hover-stay-time="100">
	...
	export default {
		...
		computed: {
			...
			ctrlList() {
				...
				const { accept, hang, micro_phone, speaker: ctrl_speaker, switch_voice, switch_camera } = ctrl(rtc_type, rtc_status, microPhone, speaker)
				let ctrl_list = []
				if (rtc_type === 'voice') {
