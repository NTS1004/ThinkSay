<template>
	<view>
		...
		<view class="friend-item" hover-class="activeItem" v-for="item in list" :key="item.id" @tap="$emit('Tap', { ...item, ...friends_record_info[item.id] })">
			...
			<view class="item-info">
				...
				<view class="info-right" v-if="type === 'recrod'">
					<u-button type="primary" :custom-style="{ flex: 1, width: '100rpx', height: '60rpx', fontSize: '28rpx' }" v-if="item.status === 'verify'" @tap="putFriendAccept(item)">接受</u-button>
...
export default {
	...
	methods: {
		async putFriendAccept(item = {}) {
			...
			try {
				await funcPutFriendAccept({ friendId: item.id, msg: item.msg, info: Object.assign({}, item, {name: this.Name(item)}) })
				item.status = 'friend'
			} catch (err) {
				console.log(err)
			}
